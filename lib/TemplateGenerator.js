...
class TemplateGenerator {
   ...
   */
  _create( options = {} ) {
    const {name, type, actions, index, test} = options;
    const filesType = config.getConfigFile().filesType;
    this._createDirectory(this._getDirPath(type), { name, actions, filesType, index, test }, filesType);
  }
  ...
  _createDirectory( dirPath, data, fileTypes ) {
    fs.readdir(dirPath, ( err, dir ) => {
      ...
      // If index option is not provided we must not create the index.js
      // file, thus we remove it from the blueprints array.
      if (!data.index) {
        dir = dir.filter(d => !d.includes("index"));
      }

      // If test option is not provided we must not create the spec
      // file, thus we remove it from the blueprints array.
      if (!data.test) {
        dir = dir.filter(d => !d.includes("spec"));
      }
